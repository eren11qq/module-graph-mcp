...
**测试状态判定**：主判定读 `coverage/coverage-summary.json`（vitest/jest 覆盖率报告，**存在即通过——MVP 不设覆盖率阈值**；agent 通过 `report_test_run` 上报失败运行后，报告内文件转红）；没有覆盖率数据时按命名约定兜底——测试文件 = 文件名以 `.test`/`.spec` 结尾，或位于任意 `__tests__/` 段下；命中同名测试文件的源码模块视为「有测试未跑」，否则「未测」。
...
- 布局：力导向 fcose（唯一布局引擎；早期的层级布局方案已裁定弃用）
- **排列双模式**（ADR 0004 + 同日修正 R2/D3）：顶栏「排列：聚类/区域」分段开关，per-root 记忆，**默认聚类**。**聚类** = Louvain 依赖社区 + 黄金角螺旋地盘的确定性海报（求解不读拖拽存档，同图重解逐位全等）；**区域** = 现行目录区域 + 罗盘海报。两模式收尾统一走全局 `separateAllBalls`——任意两球边到边 ≥32px 硬保证（含跨聚类对与游离球）；共用一份最后稳定布局互为种子，只改变空间排布、所有着色通道不变
