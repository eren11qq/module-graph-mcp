...
## 2026-09-05 死词汇清扫轮（架构评审候选 #6）

- **收窄朝被读的一侧** —— 退役机制（ADR 0002 目录折叠、ADR 0003 模块级
  边）留下的 interface 词条是导航陷阱：`CyPalette.edge.moduleColor` 零
  消费者仍占型面、`THEME.collapse.minFiles` 的唯一「引用」是两处过期
  测试注释在描述已不存在的 folds 行为、`shortLabel` 尾斜杠分支是死输入
  类。#6 三者全删，并用形状钉（edge 键全等、THEME 无 collapse）把
  「不许回潮」变成测试面——比留着当纪念便宜，比只删不钉诚实。

## 2026-09-05 工具元数据化轮（架构评审候选 #7）

- **策略是元数据，集合是派生品** —— 与注册表平行手维护的名单，其维护成本
  是记忆、其失败模式是静默：忘进基线闸门 = 对部分图报冤「module not
  found」、忘进只读屏蔽 = 权限洞、新事件忘进 relay 白名单 = 副实例的活动
  无声消失。#7 把「谁写数据（`mutating`）」「谁读图内容
  （`contentDependent`）」上移成 `ToolDef` 必填 bit（14 定义被编译器逐个
  点名表态），relay 侧改 `satisfies Record<GraphEvent['type'], 'fwd' |
  'hold'>` 穷尽映射——三张消费名单全部派生，漏登记从运行时事故变成编译
  错误。删名单前先钉等值：派生集合与历史名单（7/9/6）逐名相等
  （`tests/tool-policy.test.ts`），恒等证明过了才敢删源头。
