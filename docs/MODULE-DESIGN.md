...
| `theme` | `THEME` / `MOTION` / `CHROME` / `CY_PALETTE` / `diameterOf` / `shortLabel` / `reviewColor` | 视觉与动效常量单一事实源（边、节点半径、fcose 参数、单主题色板含 AI 评审环三色——#5 起 light 色板与「当前主题」全局已删，与 styles.css `[data-theme="dark"]` token 由等值钉锁死、viewing 紫与板块 plate 三色、`THEME.layout` 罗盘几何、`THEME.areas` 区域视觉通道——tests 缩放与跨区线细淡）；状态词汇已迁往 test-states | 423 |
